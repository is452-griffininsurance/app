...
class InsuranceOwnedTable extends Component {
  ...
  async loadDBData() {
    const temp = await getAllInsurance();
    this.setState({
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sortOrder:
          sortedInfo.columnKey === "contract_address" && sortedInfo.order,
        ellipsis: true,
      ...
      },
      {
        ...
        sortOrder:
          sortedInfo.columnKey === "insurance_type" && sortedInfo.order,
        ellipsis: true,
        render: (insuranceType) => {
          if (insuranceType === "Car") {
            return (
              <Tag color="blue" key={insuranceType}>
                {insuranceType}
              </Tag>
            );
          }
          if (insuranceType === "flight_delay") {
            return (
              <Tag color="orange" key={insuranceType}>
                <Tooltip placement="topLeft" title="Flight Delay">
                  Flight Delay
                </Tooltip>
              </Tag>
            );
          ...
          if (record.percent_insured === 1) {
            return (
              <Tag color="red" key="COMPLETED">
                <Tooltip placement="topLeft" title="COMPLETED">
                  COMPLETED
                </Tooltip>
              </Tag>
            );
    ...
    const columnsInsurers = [
      {
        ...
        render: (walletAddr) => (
          <Tooltip placement="topLeft" title={walletAddr}>
            <div style={{ color: "rgb(121, 121, 121)" }}>{walletAddr}</div>
          </Tooltip>
        ),
        width: "80%",
      },
            ...
            expandedRowRender: (record) => (
              <>
                ...
                <Row>
                  <Col span={15}>
                    <DescriptionItem
                      title="Insured Wallet Address"
                      content={record.insured_wallet_addr}
                    />
                  </Col>
                </Row>
                ...
                <Row>
                  <Col span={15}>
                    <Progress
                      percent={(record.percent_insured * 100).toFixed(2)}
                      status="active"
                      style={{ marginLeft: "10%" }}
                    />
                  </Col>
                ...
                <Row>
                  <Col span={15}>
                    <p className="site-description-item-profile-p">Insurers</p>
                  </Col>
                </Row>
                <Row>
                  <Col style={{ width: "90%" }}>
                    <Table
                      columns={columnsInsurers}
                      dataSource={record.insurers}
                      size="small"
                      pagination={false}
                      bordered
                    />
                ...
                <Row>
                  <Col span={12}>
                    <DescriptionItem
                      ...
                      content={record.coverage_amount}
                    />
                  </Col>
                  <Col span={12}>
                    <DescriptionItem
                      title="Premium Amount"
                      content={record.premium_amount}
                    />
                  </Col>
                  <Col span={12}>
                    <DescriptionItem
                      title="Max Insured Amount"
                      content={record.max_insured_amount}
                    />
                  </Col>
                  <Col span={12}>
                    <DescriptionItem
                      title="Min Insured Amount"
                      content={record.min_insured_amount}
                    />
                  </Col>
                </Row>
                <Divider style={{ marginBottom: "5px", marginTop: "15px" }} />
